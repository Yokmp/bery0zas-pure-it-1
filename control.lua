...
function load()
	...
	POLLUTE_FROM_BOXES_ENABLED = settings.startup["bery0zas-pure-it-pollutefromboxes"].value --[[@as boolean]]
	COLLECTED_POLLUTION_AMOUNT = settings.startup["bery0zas-pure-it-amountofcollectedpollution"].value --[[@as number]]
end


  -- script.on_event({ defines.events.on_built_entity, defines.events.on_robot_built_entity }, on_entity_created)
  -- script.on_event(
  -- { defines.events.on_pre_player_mined_item, defines.events.on_pre_robot_mined_item, defines.events.on_entity_died },
  --   pre_entity_removed)
  -- script.on_event(
...
-- local tower = game.surfaces[1].find_entities_filtered{name = "bery0zas-air-suction-tower-1"}
